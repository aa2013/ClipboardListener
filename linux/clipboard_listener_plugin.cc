...
#include "clipboard_listener_plugin_private.h"

#define CLIPBOARD_LISTENER_PLUGIN(obj)                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), clipboard_listener_plugin_get_type(), \
                              ClipboardListenerPlugin))

struct _ClipboardListenerPlugin
{
...
G_DEFINE_TYPE(ClipboardListenerPlugin, clipboard_listener_plugin, g_object_get_type())

// Called when a method call is received from Flutter.
static void clipboard_listener_plugin_handle_method_call(
    ClipboardListenerPlugin *self,
...
static void clipboard_listener_plugin_dispose(GObject *object)
{
  G_OBJECT_CLASS(clipboard_listener_plugin_parent_class)->dispose(object);
}

static void clipboard_listener_plugin_class_init(ClipboardListenerPluginClass *klass)
{
  G_OBJECT_CLASS(klass)->dispose = clipboard_listener_plugin_dispose;
}

static void clipboard_listener_plugin_init(ClipboardListenerPlugin *self) {}

static void method_call_cb(FlMethodChannel *channel, FlMethodCall *method_call,
                           gpointer user_data)
{
  ClipboardListenerPlugin *plugin = CLIPBOARD_LISTENER_PLUGIN(user_data);
  clipboard_listener_plugin_handle_method_call(plugin, method_call);
}

void clipboard_listener_plugin_register_with_registrar(FlPluginRegistrar *registrar)
{
  ClipboardListenerPlugin *plugin = CLIPBOARD_LISTENER_PLUGIN(
      g_object_new(clipboard_listener_plugin_get_type(), nullptr));
...
{
  ClipboardListenerPlugin *plugin = CLIPBOARD_LISTENER_PLUGIN(data);
